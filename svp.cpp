...
void LLL(Matrix& m, vector<double>& coeffs, vector<double>& norms, size_t& cols, const size_t& dim){
    ...
    while (k < cols){
        ...
        norms[k] = dot(m(k), m(k), dim);
        if (k == 1) norms[0] = dot(m(0), m(0), dim);
        ...
        if ((0.99 - (coeffs[coeffInd(k, k-1)] * coeffs[coeffInd(k, k-1)])) * norms[k - 1] > norms[k]){
            swap(m(k), m(k-1));
            --k;
            if (k < 1) k = 1;
        } else ++k;
