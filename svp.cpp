...
void LLL(Matrix* const m, vector<double>* const coeffs, vector<double>* const gNorms,
        vector<double>* const norms, size_t* const cols, const size_t& dim) {
    int k = 1;
    while (k < *cols) {
        ...
        for (int j = 0; j < k; ++j) {
            ...
            (*gNorms)[k] -= (s * s) * (*gNorms)[j];   // Calculate all Gram-Schmidt norms
            (*coeffs)[kCoeffStart + j] = s;    // Calculate all the Gram-Schmidt coefficients
        }
        ...
        for (int j = k-1; j>=0; --j) {
            ...
            if (abs(mu) > 0.5) {    // If true then the reduction calculations would be useless
                changed = true;
                mu = round(mu);
        ...
        if ((0.999 - (checkCoeff * checkCoeff)) * (*gNorms)[k - 1] > (*gNorms)[k]) {    // High delta chosen ensures termination
            swap((*m)(k), (*m)(k-1));
...
double Enumerate(Matrix* const m, vector<double>* const coeffs, vector<double>* const gNorms,
...
bool Init(Matrix* const m, vector<double>* const coeffs, vector<double>* const gNorms,
            vector<double>* const norms, const size_t& cols, const size_t& dim) {
